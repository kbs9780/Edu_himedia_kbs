...
<%@ page info="/WEB-INF/view/front/sale/shop/reptile/list.jsp" %>
			...
			<article class="txtCenter">
				<div class="brdSearchArea" style="margin: 60px; margin-left: 100px; margin-bottom: 15px">
					<select id="pet_items" name="pet_items" required>
				        ...
				        <option value="41" <c:if test="${paging.pet_items == 41}">selected</c:if>>사료</option>
				        <option value="42" <c:if test="${paging.pet_items == 42}">selected</c:if>>간식</option>
				        <option value="43" <c:if test="${paging.pet_items == 43}">selected</c:if>>하우스</option>
				        <option value="44" <c:if test="${paging.pet_items == 44}">selected</c:if>>기타</option>
				    </select>
