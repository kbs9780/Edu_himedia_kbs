...
<%@ page info="/WEB-INF/view/front/sale/shop/fish/list.jsp" %>
		...
		<article class="txtCenter">
			<div class="brdSearchArea" style="margin: 60px; margin-left: 100px; margin-bottom: 15px">
				<select id="pet_items" name="pet_items" required >
			        ...
			        <option value="51" <c:if test="${paging.pet_items == 51}">selected</c:if>>사료</option>
			        <option value="52" <c:if test="${paging.pet_items == 52}">selected</c:if>>간식</option>
			        <option value="53" <c:if test="${paging.pet_items == 53}">selected</c:if>>하우스</option>
			        <option value="54" <c:if test="${paging.pet_items == 54}">selected</c:if>>기타</option>
			    </select>
